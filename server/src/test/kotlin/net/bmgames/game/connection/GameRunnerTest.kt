...
class GameRunnerTest : FunSpec({
    ...
    test("Uninvited player should not be able to join") {
        ...
        connection.value.shouldBeTypeOf<String>()
            .shouldContainIgnoringCase("not invited")
    }
    ...
    test("Connected player should not join a second time") {
        ...
        invalid.value.shouldBeTypeOf<String>()
            .shouldContainIgnoringCase("already connected")
    }
